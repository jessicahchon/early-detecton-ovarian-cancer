# RF FS + CERP Evaluation (Corrected)
...
print(f"\n{'='*80}")
print("RF FS + CERP (1-SE pruning, Weighted Voting)")
...
for repeat in range(N_REPEATS):
    ...
    for fold_idx, (tv_idx, te_idx) in enumerate(outer_cv.split(X, y)):
        ...
        # RF Feature Selection (unscaled - tree-based)
        rf_search = RandomizedSearchCV(
            RandomForestClassifier(random_state=42, n_jobs=-1, bootstrap=True),
            param_distributions=RF_SEL_PARAM_DIST, n_iter=N_ITER_SEARCH,
        ...
        selector = SelectFromModel(rf_search.best_estimator_, threshold='mean')
...
wt_df.to_csv('RF_Selection_CERP_weighted_results.csv', index=False)
maj_df = pd.DataFrame(all_results_maj)
maj_df.to_csv('RF_Selection_CERP_majority_results.csv', index=False)

mean_tpr_wt = np.mean(all_tprs_wt, axis=0); mean_tpr_wt[-1] = 1.0
pd.DataFrame({'FPR': mean_fpr, 'TPR': mean_tpr_wt}).to_csv(
    'RF_Selection_CERP_weighted_ROC_Data.csv', index=False)
mean_tpr_maj = np.mean(all_tprs_maj, axis=0); mean_tpr_maj[-1] = 1.0
pd.DataFrame({'FPR': mean_fpr, 'TPR': mean_tpr_maj}).to_csv(
    'RF_Selection_CERP_majority_ROC_Data.csv', index=False)

rf_df = pd.DataFrame(rf_params_history)
with open('RF_Selection_CERP_RF_Param_Frequencies.txt', 'w') as f:
    f.write("RF Selector Parameter Frequency\n")
    f.write(f"Total folds: {len(rf_params_history)}\n\n")
    for col in rf_df.columns:
        f.write(f"Parameter: {col}\n")
        vc = rf_df[col].astype(str).value_counts()
        for val, cnt in vc.items():
            f.write(f"  {val}: {cnt} ({cnt/len(rf_df)*100:.1f}%)\n")
        f.write("\n")

r_counts, ens_counts = Counter(), Counter()
for p in cerp_params_history:
    r_counts[p['r']] += 1; ens_counts[p['n_ensembles']] += 1
with open('RF_Selection_CERP_CERP_Param_Frequencies.txt', 'w') as f:
    f.write(f"r: {dict(r_counts)}\nn_ensembles: {dict(ens_counts)}\n")

# === Summary ===
for label, res_df in [("Majority", maj_df), ("Weighted", wt_df)]:
    print(f"\n{'='*60}")
    print(f"RF FS + CERP {label} (Mean +/- SD, 70 folds)")
    print(f"{'='*60}")
...
print(f"  Features: {wt_df['N_Selected'].mean():.0f} +/- {wt_df['N_Selected'].std():.0f}")
