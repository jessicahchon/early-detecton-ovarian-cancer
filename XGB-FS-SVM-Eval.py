...
# SVM evaluator search space
SVM_EVAL_PARAM_DIST = {
    'C': loguniform(0.1, 100),
    'gamma': loguniform(0.0001, 1.0),
    'kernel': ['linear', 'rbf']
}
SVM_EVAL_FIXED = {'probability': True, 'random_state': 42}
...
for r in range(N_REPEATS):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
        ...
        eval_search = RandomizedSearchCV(
            SVC(**SVM_EVAL_FIXED),
            param_distributions=SVM_EVAL_PARAM_DIST,
...
with open(f'XGB_Selection_SVM_Param_Frequencies.txt', 'w') as f:
    f.write(f"SVM Evaluator Parameter Frequency Analysis\n")
