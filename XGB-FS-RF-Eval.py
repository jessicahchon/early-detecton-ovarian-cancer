...
# RF evaluator search space
RF_EVAL_PARAM_DIST = {
    'n_estimators': randint(500, 1001),
    'max_depth': [3, 5, 7, 9],
    'max_features': ['sqrt', 0.5]
}
RF_EVAL_FIXED = {'random_state': 42, 'n_jobs': -1, 'bootstrap': True}
...
for r in range(N_REPEATS):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
        ...
        # RF Evaluator tuning on selected features
        inner_cv_eval = StratifiedKFold(n_splits=N_INNER, shuffle=True,
                                        random_state=RANDOM_SEED + r)
        eval_search = RandomizedSearchCV(
            RandomForestClassifier(**RF_EVAL_FIXED),
            param_distributions=RF_EVAL_PARAM_DIST,
