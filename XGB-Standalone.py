...
# XGB evaluator search space (same range, independently tuned)
XGB_EVAL_PARAM_DIST = {
    'n_estimators': randint(300, 601),
    'max_depth': [3, 5, 7, 9],
    'learning_rate': loguniform(0.001, 0.1)
...
for r in range(N_REPEATS):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
        ...
        # STEP 1: XGB Feature Selection (per-fold tuned)
        ...
        # STEP 2: XGB Evaluator tuning on selected features
        inner_cv_eval = StratifiedKFold(n_splits=N_INNER, shuffle=True,
                                        random_state=RANDOM_SEED + r)
        eval_search = RandomizedSearchCV(
            XGBClassifier(**XGB_EVAL_FIXED),
            param_distributions=XGB_EVAL_PARAM_DIST,
...
with open(f'XGB_Selection_{model_name}_Param_Frequencies.txt', 'w') as f:
